...
def denoise_GNN(dataset, GNN_checkpoint, layer, num_epochs, num_hidden, lr, K, 
    ...
    # Model
    model = ARMA.Net(num_in=dataset.num_features, num_hid=16, num_out=dataset.num_classes, num_stacks=[3,3], num_layers=[2,2], shared_weights=[True,True], dropout=[0.25,0.25], act=None).to(device)
    ...
    train_acc, val_acc, test_acc = ARMA.test(model, data, z, layer)
    print("\nVerify loading:")
    ...
    train_acc, val_acc, test_acc = ARMA.test(model, data, z_tilde, layer)
