...
class B_RBM(nn.Module):
    ...
    def __init__(self, v_type, num_v, num_h):
        """
            Restricted Boltzmann Machine with Bernoulli hidden units

            Args:
                - v_type (str): Gaussian or Bernoulli visible units
                - num_v (int): Number of visible units
                - num_h (int): Number of hidden units
    ...
    def Ph_v(self, v):
        """
          ...
          Args:
            - v: Visible units values

          Returns:
            - hid_probs: Probabilities of hidden units
            - hid_states: Sample drawn from hid_probs
        """
    ...
    def Pv_h(self, h):
        """
          ...
          Args:
            - h: Hidden units values

          Returns:
            if Bernoulli:
              - vis_probs: Probabilities of visible units
              - vis_states: Sample drawn from vis_probs
            if Gaussian:
              - vis_distn: Probability distribution that visible units follow
              - vis_states: Sample drawn from vis_distn
        """
        if self.v_type == "Gaussian":
    ...
    def CD(self, v0, epoch, eta, K=1):
        """
            ...
            Args:
                - v0 (num_patterns, num_v): Patterns to store
    ...
    def delta(self, v, ph):
        """
            ...
            Args:
                - v (num_patterns, num_v): Patterns (either original or reconstructions)
                - ph (num_patterns, num_h): Hidden unit probabilities

            Returns:
                - nabla_bv (num_patterns, num_v): Gradient w.r.t visible bias
                - nabla_W (num_v, num_h): Gradient w.r.t weight matrix
                - nabla_bh (num_patterns, num_h): Gradients w.r.t hidden bias
                - nabla_sigma (num_patterns, num_v): Gradients w.r.t standard deviation vector
        """
    ...
    def train_RBM(self, num_epochs, data, eta, K, batch_size, save_chkpt, device):
        """
            ...
            Args:
                ...
                - data (num_patterns, num_v): Number of patterns to store
                - eta (float): Learning rate
                - K (int): Number of contrastive divergence steps
                - batch_size (int): Batch size
                - save_chkpt: Absolute path to the file to save the model state dictionary
                - device: Torch device

            Returns:
                - l2_list: List of L2 distances between the original patterns and their reconstructions
        """
        self.train()
